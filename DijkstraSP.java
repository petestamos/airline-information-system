...
public class DijkstraSP {
     ...
     * @param  primary if it's based off the primary weight or secondary weight
     * @throws IllegalArgumentException if an edge weight is negative
     * @throws IllegalArgumentException unless 0 &le; <tt>s</tt> &le; <tt>V</tt> - 1
     */
    public DijkstraSP(EdgeWeightedDigraph G, int s, boolean primary) {
    ...
    private void relax2(DirectedEdge e) {
        int v = e.from(), w = e.to();
        if (distTo[w] > distTo[v] + e.weightTwo()) {
            distTo[w] = distTo[v] + e.weightTwo();
            edgeTo[w] = e;
            if (pq.contains(w)) pq.decreaseKey(w, distTo[w]);
            else                pq.insert(w, distTo[w]);
        }
    ...
    public Iterable<DirectedEdge> pathTo(int v) {
        ...
        path.push(new DirectedEdge(0, 0, length, 0));
        return path;
    }

    public Iterable<DirectedEdge> pathToTwo(int v) {
        if (!hasPathTo(v)) return null;
        Stack<DirectedEdge> path = new Stack<DirectedEdge>();
