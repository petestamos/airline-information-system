...
public class DirectedEdge {
     ...
     */
    public DirectedEdge(int v, int w, double weight, double weightTwo) {
        if (v < 0) throw new IndexOutOfBoundsException("Vertex names must be nonnegative integers");
        if (w < 0) throw new IndexOutOfBoundsException("Vertex names must be nonnegative integers");
        if (Double.isNaN(weight)) throw new IllegalArgumentException("Weight is NaN");
        if (Double.isNaN(weightTwo)) throw new IllegalArgumentException("WeightTwo is NaN");
        this.v = v;
        this.w = w;
        this.weight = weight;
    ...
    /**
     * Returns the weightTwo of the directed edge.
     * @return the weightTwo of the directed edge
     */
